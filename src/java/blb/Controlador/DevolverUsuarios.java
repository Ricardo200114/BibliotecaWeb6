...
@WebServlet(name = "DevolverUsuarios", urlPatterns = {"/DevolverUsuarios"})
public class DevolverUsuarios extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (menu.equals("Devolver")) {
            ...
            request.getRequestDispatcher("/DevolverUsuario/DevolverMaterial.jsp").forward(request, response);
    ...
    private void listarMaterial(HttpServletRequest request) {
        try {
            ...
            String nCarnet = request.getParameter("noCarnet");

            List lista = pd.listarPrestamoPorId(nCarnet);
        ...
        try {
            ...
            if (codMaterial.matches("LIB\\d{4}")) {
                ...
                if (li == null) {
                    ...
                    request.setAttribute("mensaje", "No se encontro material con código " + codMaterial);
                    request.getRequestDispatcher("DevolverUsuarios?menu=Devolver&accion=Listar&noCarnet="+ us.getNoCarnet()).forward(request, response);
                    return;
                }
                ...
                if (p == null) {
                    ...
                    request.setAttribute("config", "alert alert-danger");
                    request.setAttribute("mensaje", "No se encontro el préstamo del material a devolver...");
                    request.getRequestDispatcher("DevolverUsuarios?menu=Devolver&accion=Listar&noCarnet="+ us.getNoCarnet()).forward(request, response);
                    return;
                }
                ...
                if (us.getOficio().equals("ESTUDIANTE") && dias > a.getDiaEstudiante()) {
                    ...
                    request.setAttribute("mensaje", "Devolucion exitosa, se ha aplicado recargo por devolución tardia de $" + recargo);
                    request.getRequestDispatcher("DevolverUsuarios?menu=Devolver&accion=Listar&noCarnet="+ us.getNoCarnet()).forward(request, response);
                }
            ...
            //---SI EL CODIGO DEL MATERIAL A DEVOLVER NO COINCIDE CON NINGUNA DE LAS OPCIONES ENTONCES MENSAJE DE ERROR APARCE----        
                ...
                request.setAttribute("config", "alert alert-warning");
                request.setAttribute("mensaje", "El código de material ingresado es incorrecto, vuelve a intentarlo... ");
                request.getRequestDispatcher("DevolverUsuarios?menu=Devolver&accion=Listar&noCarnet="+ us.getNoCarnet()).forward(request, response);
